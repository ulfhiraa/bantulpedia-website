...
export default function StatusGiziPage() {
  ...
  const chartDataRaw = [
    ...
    { region: "Sanden", total: 98, male: 54, female: 44, prev: 8.7 },
    { region: "Kretek", total: 125, male: 68, female: 57, prev: 10.36 },
    { region: "Pundong", total: 206, male: 123, female: 83, prev: 12.68 },
    { region: "Bambanglipuro", total: 112, male: 59, female: 53, prev: 6.8 },
    ...
    { region: "Pleret", total: 139, male: 70, female: 69, prev: 9.4 },
  ...
  const processedData = useMemo(() => {
    return chartDataRaw.map((d) => {
      ...
      const adjPrev = parseFloat(
        (
          d.prev *
          (factor === 1 ? 1 : factor === 0.6 ? 0.8 : 1.1)
        ).toFixed(2)
      );
  ...
  const calculateMax = (data, key) =>
    Math.ceil(Math.max(...data.map((d) => d[key])) * 1.2) || 100;

  const maxTotal = calculateMax(processedData, "total");
  const maxMeasured = calculateMax(processedData, "measured");
  const maxGenderTotal =
    Math.ceil(
      Math.max(...processedData.map((d) => Math.max(d.male, d.female))) * 1.2
    ) || 100;
  const maxGenderMeasured =
    Math.ceil(
      Math.max(
        ...processedData.map((d) => Math.max(d.measuredMale, d.measuredFemale))
      ) * 1.2
    ) || 100;
  const maxProblemPrev = calculateMax(processedData, "prev");

  const currentMaxTotal = activeViewTotal === "total" ? maxTotal : maxMeasured;
  const currentMaxGender =
    activeViewGender === "total" ? maxGenderTotal : maxGenderMeasured;
  ...
  // --- DATA KARTU METRIK ---
  // include colorFrom/colorTo for gradient classes (Tailwind JIT)
  const metrics = [
    {
      key: "stunting",
      ...
      trend: "down",
      icon: "📏",
      definitionTitle: "STUNTING",
      definition:
        "Kurang gizi kronis, asupan kurang lama. Angka: [Pendek] + [Sangat Pendek]",
      colorFrom: "from-teal-600",
      colorTo: "to-teal-800",
    },
    {
      key: "wasting",
      title: "Balita Wasting",
      total: "2.396",
      male: "1.438",
      female: "958",
      prevalence: "5,77%",
      trend: "up",
      icon: "⚖️",
      definitionTitle: "WASTING",
      definition:
        "Kekurangan gizi akut, berat rendah dibanding tinggi. Angka: [Gizi Kurang] + [Gizi Buruk]",
    ...
    },
    {
      ...
      trend: "down",
      icon: "🩺",
      definitionTitle: "UNDERWEIGHT",
      definition:
        "Berat badan di bawah rata-rata usia. Angka: [Berat Kurang] + [Sangat Kurang]",
  ...
  return (
    <div className="min-h-screen bg-slate-50 flex flex-col font-poppins">
      ...
      <div className="max-w-7xl mx-auto w-full px-4 md:px-8 lg:px-12 mt-8 relative z-20 pb-20">
        {/* header row (button + title + spacer) */}
        <div className="mb-6 grid grid-cols-3 items-center gap-4">
          ...
          <div className="flex justify-center">
            <h1 className="text-2xl md:text-3xl lg:text-xl font-semibold text-slate-900 leading-tight text-center">
        ...
        <div className="bg-white rounded-2xl shadow-lg border border-slate-100 p-4 md:p-6 mb-10 flex flex-col md:flex-row items-center justify-between gap-4">
          <div className="flex gap-4 w-full md:w-auto">
            <div className="w-1/2 md:w-48">
              <label className="text-xs font-semibold text-slate-500 mb-1 block pl-1">
                Tahun
              </label>
              <div className="relative">
                <select
                  ...
                  className="w-full appearance-none bg-white border border-slate-300 text-slate-700 rounded-full px-4 py-2 pr-8 focus:ring-2 focus:ring-[#148448]/40 outline-none"
                >
                  {YEARS.map((y) => (
          ...
          <div className="flex-shrink-0">
            <img src={logoKab} alt="Bantul" className="w-12 h-12 object-contain" />
          </div>
        </div>

        {/* HEADER: Hasil Pengukuran */}
        <div
          className="bg-[#148448]/60 border border-[#148448]/60 rounded-lg py-3 px-4 text-center mb-8 shadow-sm"
          role="region"
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-12">
          ...
          <div className="bg-[#C4E2D1] rounded-xl p-6 text-center shadow-sm border border-[#148448]/20 flex flex-col items-center justify-center h-40">
            ...
            <div className="text-sm font-medium text-black px-4">
              Jumlah Balita yang di ukur & Ditimbang
            </div>
          </div>
          ...
          <div className="bg-[#C4E2D1] rounded-xl p-6 text-center shadow-sm border border-[#148448]/20 flex flex-col items-center justify-center h-40">
            ...
            <div className="text-sm font-medium text-black px-4">
              Persentase Balita yang di ukur & Ditimbang
            </div>
          </div>
        </div>
        ...
        {/* PROBLEM METRIC DEFINITION CARDS */}
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
          {metrics.map((m) => (
            <div
              key={m.key + "def"}
              className="bg-white rounded-2xl p-4 shadow-sm border border-slate-200 flex items-start gap-4"
            >
              <div className="w-12 h-12 rounded-lg bg-orange-50 flex items-center justify-center text-2xl flex-shrink-0 border border-orange-100">
                {m.icon}
              </div>
              <div>
                ...
                <p className="text-xs text-slate-500 leading-snug">{m.definition}</p>
              </div>
            </div>
          ))}
        </div>

        {/* PROBLEM METRIC BIG CARDS (SESUIKAN DENGAN GAMBAR) */}
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-16">
          {metrics.map((m) => (
            <div
              key={m.key}
              className={`relative overflow-hidden rounded-3xl shadow-lg bg-gradient-to-b ${m.colorFrom} ${m.colorTo} text-white`}
            >
              <div className="p-6 pb-24 relative z-10 text-center">
                <h3 className="text-xl font-semibold opacity-100">{m.title}</h3>
                <div className="text-5xl font-extrabold mt-3 tracking-tight">{m.total}</div>
                <div className="text-[10px] font-bold tracking-wider opacity-80 uppercase mt-1">
                  TOTAL BALITA {m.definitionTitle}
                </div>

                <div className="mt-6 flex gap-3 text-left">
                  <div className="bg-white/20 backdrop-blur-sm rounded-lg px-3 py-2 flex items-center gap-2 flex-1">
                    <span className="text-xl">👦</span>
                    <div>
                      <div className="text-[9px] uppercase opacity-90 leading-none mb-0.5">
                        Laki-laki
                      </div>
                      <div className="font-bold text-sm leading-tight">{m.male}</div>
                    </div>
                  </div>
                  ...
                  <div className="bg-white/20 backdrop-blur-sm rounded-lg px-3 py-2 flex items-center gap-2 flex-1">
                    ...
                    <div>
                      ...
                      <div className="font-bold text-sm leading-tight">{m.female}</div>
                    </div>
                  </div>
                </div>
              </div>

              <div className="absolute bottom-4 left-4 right-4 bg-white rounded-xl p-4 shadow-lg text-slate-800 flex items-center justify-between">
                <div>
                  <div className="text-xs font-bold text-slate-400 uppercase">Prevalensi:</div>
                  <div className={`text-3xl font-extrabold ${m.key === 'stunting' ? 'text-teal-600' : m.key === 'wasting' ? 'text-cyan-600' : 'text-sky-600'}`}>{m.prevalence}</div>
                </div>

                <div className="flex items-center gap-2 text-right">
                  <div className="flex flex-col items-end">
                    <div className={`w-6 h-6 rounded-full flex items-center justify-center text-white mb-1 ${m.trend === 'down' ? 'bg-teal-500' : 'bg-rose-500'}`}>
                      {m.trend === 'down' ? <ArrowDown size={14} /> : <ArrowUp size={14} />}
                    </div>
                  </div>
                  <div className="text-[10px] text-slate-500 leading-tight w-16">
                    {m.trend === 'down' ? 'Menurun' : 'Naik'} dari Tahun Lalu
                  </div>
                </div>
              </div>
            </div>
          ...
          <div className="flex flex-col sm:flex-row gap-3 w-full md:w-auto">
            ...
            <div className="relative min-w-[220px]" ref={dropdownRef}>
              <button
                ...
                className="w-full bg-white border border-slate-300 rounded-lg px-4 py-2.5 shadow-sm flex items-center justify-between hover:border-[#148448]/60 transition-colors"
              >
                ...
                <div className="absolute top-full mt-2 left-0 right-0 bg-white border border-slate-200 rounded-xl shadow-xl z-50 max-h-80 overflow-hidden flex flex-col">
                  <div className="p-3 border-b border-slate-100 relative">
                    ...
                    <input
                      ...
                      className="w-full pl-9 pr-3 py-2 bg-slate-50 rounded-lg text-sm outline-none focus:ring-1 focus:ring-[#148448]/40"
                      value={searchTerm}
                      ...
                      filteredOptions.map((region, idx) => {
                        ...
                        return (
                          ...
                          >
                            <div
                              className={`w-5 h-5 rounded border flex items-center justify-center mr-3 ${
                                isSelected ? "bg-[#148448]/60 border-[#148448]/60" : "border-slate-300"
                              }`}
                            >
        ...
        {/* GRAFIK 1: JUMLAH BALITA (sederhana, berbasis div) */}
        <div className="bg-white rounded-3xl shadow-xl border border-slate-100 overflow-hidden mb-12">
          <div className="p-6 md:p-8 border-b border-slate-100 flex flex-col md:flex-row justify-between items-center gap-4">
            <h3 className="text-xl md:text-2xl font-bold text-slate-800">
              {activeViewTotal === "total"
                ? "Jumlah Balita menurut Kapanewon"
                : "Jumlah Balita diukur & ditimbang menurut Kapanewon"}
            </h3>
            <div className="bg-slate-100 p-1.5 rounded-xl flex text-xs md:text-sm font-medium relative">
              <button
                onClick={() => setActiveViewTotal("total")}
                className={`px-5 py-2.5 rounded-lg transition-all duration-200 relative z-10 ${
                  activeViewTotal === "total"
              ...
              <button
                onClick={() => setActiveViewTotal("measured")}
                className={`px-5 py-2.5 rounded-lg transition-all duration-200 relative z-10 ${
                  activeViewTotal === "measured"
                    ? "text-slate-800 bg-white shadow-sm font-bold"
                    : "text-slate-500 hover:text-slate-700"
                }`}
              >
                Jumlah di ukur & ditimbang
              </button>
            </div>
          </div>
            ...
            <div className="min-w-[800px] h-[450px] relative flex flex-col justify-end pt-10 pb-20">
              <div className="absolute inset-0 flex flex-col justify-between pointer-events-none z-0 pb-20 pt-10">
                {[100, 75, 50, 25, 0].map((val, idx) => (
                  <div
                    key={idx}
                    className="w-full border-b border-slate-200 border-dashed relative"
                  >
                    <span className="absolute -top-3 -left-8 text-xs font-semibold text-slate-400 w-6 text-right">
                      {Math.round(currentMaxTotal * (val / 100))}
                    </span>
                  </div>
                ))}
              </div>
              ...
              <div
                className={`flex items-end h-full z-10 pl-2 pr-2 pb-1 ${
                  filteredData.length < 6 ? "justify-start gap-12" : "justify-between"
                }`}
              >
                {filteredData.map((item, index) => {
                  ...
                  return (
                    <div
                      key={index}
                      className="flex flex-col items-center justify-end h-full w-full group relative px-1 max-w-[80px]"
                    >
                      <div className="flex flex-col items-center justify-end w-full h-full relative">
                        ...
                        >
                          <span className="text-[10px] md:text-xs font-bold text-[#1e5b7e]">
                            {dataValue}
                          </span>
                        </div>
                        <div
                          className="w-6 md:w-12 rounded-t-sm transition-all duration-500 ease-out hover:opacity-90"
                          style={{ height: `${height}%`, backgroundColor: "#1e5b7e" }}
                        />
                      </div>

                      <div className="absolute -bottom-10 left-1/2 -translate-x-1/2 translate-y-full w-4 flex justify-center">
                        <div className="text-[10px] md:text-xs text-slate-600 font-medium whitespace-nowrap origin-top-left -rotate-45 mt-2 ml-1">
                          {item.region}
                        </div>
                      </div>
                    </div>
        ...
        {/* GRAFIK 2: GENDER BALITA (sederhana) */}
        <div className="bg-white rounded-3xl shadow-xl border border-slate-100 overflow-hidden mb-8">
          <div className="p-6 md:p-8 border-b border-slate-100 flex flex-col md:flex-row justify-between items-center gap-4">
            ...
            <div className="bg-slate-100 p-1.5 rounded-xl flex text-xs md:text-sm font-medium relative">
              <button
                onClick={() => setActiveViewGender("total")}
                className={`px-5 py-2.5 rounded-lg transition-all duration-200 relative z-10 ${
                  activeViewGender === "total"
                    ? "text-slate-800 bg-white shadow-sm font-bold"
                    : "text-slate-500 hover:text-slate-700"
                }`}
              >
              ...
              <button
                onClick={() => setActiveViewGender("measured")}
                className={`px-5 py-2.5 rounded-lg transition-all duration-200 relative z-10 ${
                  activeViewGender === "measured"
                    ? "text-slate-800 bg-white shadow-sm font-bold"
                    : "text-slate-500 hover:text-slate-700"
                }`}
              >
            ...
            <div className="min-w-[800px] h-[450px] relative flex flex-col justify-end pt-10 pb-20">
              <div className="absolute inset-0 flex flex-col justify-between pointer-events-none z-0 pb-20 pt-10">
                {[100, 75, 50, 25, 0].map((val, idx) => (
                  ...
                  >
                    <span className="absolute -top-3 -left-8 text-xs font-semibold text-slate-400 w-6 text-right">
                      {Math.round(currentMaxGender * (val / 100))}
                    </span>
                  </div>
                ))}
              </div>
                    ...
                    >
                      <div className="flex gap-1 items-end justify-center w-full h-full relative">
                        <div className="flex flex-col items-center justify-end h-full w-1/2 relative">
                          <span
                            className="absolute text-[9px] font-bold text-blue-600 mb-1 transition-all duration-500"
                            style={{ bottom: `${hMale}%` }}
                          >
                            {dataMale}
                          </span>
                          <div
                            className="w-full rounded-t-sm bg-blue-500 transition-all duration-500 hover:bg-blue-600"
                            style={{ height: `${hMale}%` }}
                          />
                        </div>
                        ...
                        <div className="flex flex-col items-center justify-end h-full w-1/2 relative">
                          <span
                            className="absolute text-[9px] font-bold text-pink-500 mb-1 transition-all duration-500"
                            style={{ bottom: `${hFemale}%` }}
                          >
                            {dataFemale}
                          </span>
                          <div
                            className="w-full rounded-t-sm bg-pink-500 transition-all duration-500 hover:bg-pink-600"
                            style={{ height: `${hFemale}%` }}
                          />
                        </div>
                      </div>
              ...
              <div className="border-t border-slate-100 pt-4 flex justify-end gap-6 text-xs md:text-sm text-slate-600">
                <div className="flex items-center gap-2">
                  ...
                  <span className="font-medium">Laki-laki (L)</span>
                </div>
                <div className="flex items-center gap-2">
                  <div className="w-4 h-4 bg-pink-500 rounded-sm" />
                  <span className="font-medium">Perempuan (P)</span>
                </div>
              </div>
            </div>
          </div>
        ...
        <div className="mb-6 mt-16 flex flex-col md:flex-row justify-between items-center gap-4">
          ...
          <div className="w-full md:w-auto relative">
            <div className="flex items-center bg-white border border-slate-300 rounded-lg px-4 py-2.5 shadow-sm min-w-[280px] hover:border-[#148448]/60 transition-colors focus-within:ring-2 focus-within:ring-[#148448]/20">
        ...
        <div className="bg-white rounded-3xl shadow-xl border border-slate-100 overflow-hidden mb-12">
          <div className="p-6 md:p-8 border-b border-slate-100 flex flex-col md:flex-row justify-between items-center gap-4">
            ...
            <div className="bg-slate-100 p-1.5 rounded-xl flex text-xs md:text-sm font-medium relative">
              <button
                onClick={() => setActiveProblemMetric("total")}
                className={`px-4 py-2.5 rounded-lg transition-all duration-200 ${
                  activeProblemMetric === "total" ? "text-slate-800 bg-white shadow-sm font-bold" : "text-slate-500 hover:text-slate-700"
                }`}
              >
                Total Kasus
              </button>
              ...
              <button
                onClick={() => setActiveProblemMetric("prev")}
                className={`px-4 py-2.5 rounded-lg transition-all duration-200 ${
                  activeProblemMetric === "prev" ? "text-slate-800 bg-white shadow-sm font-bold" : "text-slate-500 hover:text-slate-700"
                }`}
              >
                Prevalensi (%)
              </button>
              ...
              <button
                onClick={() => setActiveProblemMetric("gender")}
                className={`px-4 py-2.5 rounded-lg transition-all duration-200 ${
                  activeProblemMetric === "gender" ? "text-slate-800 bg-white shadow-sm font-bold" : "text-slate-500 hover:text-slate-700"
                }`}
              >
                Gender (L/P)
              </button>
              ...
              <div className="absolute inset-0 flex flex-col justify-between pointer-events-none z-0 pb-20 pt-10">
                {[100, 75, 50, 25, 0].map((val, idx) => (
                  <div key={idx} className="w-full border-b border-slate-200 border-dashed relative">
                    <span className="absolute -top-3 -left-8 text-xs font-semibold text-slate-400 w-6 text-right">
                      {Math.round(currentMaxProblem * (val / 100))}
                    </span>
                  </div>
                ))}
              </div>
                      ...
                      <div key={index} className="flex flex-col items-center justify-end h-full w-full group relative px-1">
                        <div className="flex gap-1 items-end justify-center w-full h-full relative">
                          <div className="flex flex-col items-center justify-end h-full w-1/2 relative">
                            ...
                            <div className="w-full rounded-t-sm bg-blue-500 transition-all duration-500" style={{ height: `${hMale}%` }} />
                          </div>
                          <div className="flex flex-col items-center justify-end h-full w-1/2 relative">
                            <span className="absolute text-[9px] font-bold text-pink-500 mb-1" style={{ bottom: `${hFemale}%` }}>{item.female}</span>
                            <div className="w-full rounded-t-sm bg-pink-500 transition-all duration-500" style={{ height: `${hFemale}%` }} />
                          </div>
                        </div>
                        <div className="absolute -bottom-10 left-1/2 -translate-x-1/2 translate-y-full w-4 flex justify-center">
                    ...
                    return (
                      <div key={index} className="flex flex-col items-center justify-end h-full w-full group relative px-1">
                        <div className="flex flex-col items-center justify-end w-full h-full relative">
                          ...
                          <div className="w-6 md:w-12 rounded-t-sm transition-all duration-500 ease-out hover:opacity-90" style={{ height: `${height}%`, backgroundColor: barColor }} />
                        </div>
                        <div className="absolute -bottom-10 left-1/2 -translate-x-1/2 translate-y-full w-4 flex justify-center">
                          <div className="text-[10px] md:text-xs text-slate-600 font-medium whitespace-nowrap origin-top-left -rotate-45 mt-2 ml-1">{item.region}</div>
                        </div>
